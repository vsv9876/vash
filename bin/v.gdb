...
BLD=$vashdir/BLD
#bintest=vash/vash
bintest=$BLD$dest/bin/vash

# debug env, reset my own regular setup
if [ -x $bintest ]; then
    onpath=`echo $PATH|grep -c $BLD$dest`
    if [ $onpath -eq 0 ]; then
	PATH=$BLD$dest/bin:$PATH;
	export PATH
    fi
    VASH_PATH="${BLD}$dest/lib/vash:$dest/lib/vash"
    export VASH_PATH
    VHSET="${vashdir}/etc/vhset"
    export VHSET
    VHSET_LIB="${BLD}$dest/lib/vhset:$dest/lib/vhset"
    export VHSET_LIB
    echo ' *'
