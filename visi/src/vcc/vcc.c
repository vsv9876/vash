...
typedef struct {
	...
	linptr_t siz;           /* ςαϊνες πομεϊξοηο νεστα χ βακταθ */
...
err_exit:
	fprintf(stderr, "%s: '%s'\n",
	ediag("Bad parameters count","πμοθοε λομιώεστχο παςανετςοχ"), lbpssav);
...
int co;
{
	...
	linptr_t spcnt;         /* σώετώιλ πςοβεμοχ χ λοξγε λοννεξταςιρ */
...
BEG_OF_SCAN:
	...
	if ((*from != '\0') && (*from != '=')) {
		...
		ediag(" the '=' must be after name.", " '=' ΔΟΜΦΕΞ ΒΩΤΨ ΠΟΣΜΕ ΙΝΕΞΙ.") );
		return;
	} else {
		*to++ = *from++;
	}
