/* tgoto [termcap]
**
** ДЛЯ ИСПОЛЬЗОВАНИЯ СОВМЕСТНО С LINLIB v3.0
** НАДО БЫ ПОДЛЕЧИТЬ ПРОЦЕДУРУ КОРРЕКТИРОВКИ ФОРМАТА %.
**
**/
#include <string.h>
...
/*
 * ПРОГРАММА ДЛЯ ВЫПОЛНЕНИЯ АДРЕСАЦИИ КУРСОРА
 * CM ЯВЛЯЕТСЯ СТРОКОЙ ДЛЯ ВЫПОНЕНИЯ АДРЕСАЦИИ КУРСОРА.
...
{
	static char bcount;             /* СЧЕТЧИК ВОЗВРАТОВ ДЛЯ %. */
...
casedot:
			 ...
			 */
			bcount = 0;             /* СЧЕТЧИК ВОЗВРАТОВ */
			switch( which )  {      /* СЛУЧАЕВ ДОВОЛЬНО МНОГО... */
				case '\t'    :
				case 'n'&037 : bcount++;
				case 0       :
				case 'd'&037 :
			...
			while(bcount--) {       /* ДоПИСЫВАЕМ ВОЗВРАТ.. */
